...
var Range = function(start=0, end=start, step) {
    this.start=start;
    this.end = end;
    this.step = step || 1;
};
...
Range.prototype.size = function () {
    return Math.ceil((Math.abs(this.end-this.start)+1)/Math.abs(this.step));
};

Range.prototype.each = function (callback) {
    
    const { start, end, step } = this;
    const ascIterator = (start, end, step) => {
        for(let i=start;i<=end;i+=step){
            callback(i);
        }
    }
    const dscIterator = (start, end, step) => {
        for(let i=start;i>=end;i+=step){
            callback(i);
        }
    }
    start < end ?
    step > 0 ? ascIterator(start, end, step) : dscIterator(end, start, step) :
    step > 0 ? dscIterator(start, end, -step) : dscIterator(start, end, step)
};
